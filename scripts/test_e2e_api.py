...
async def clean_database():
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Disable triggers/FK constraints temporarily if needed, but simple DELETE works if in correct order
            await conn.execute(text("DELETE FROM alerts"))
            await conn.execute(text("DELETE FROM money_trails"))
            await conn.execute(text("DELETE FROM transactions"))
            await conn.execute(text("DELETE FROM statements"))
            await conn.execute(text("DELETE FROM analysis_tasks"))
            await conn.execute(text("DELETE FROM entity_case_appearances"))
