...
def _is_header_row(cells: list[str]) -> bool:
    ...
    return sum(1 for kw in ["date", "particulars", "narration", "debit", "credit", "balance", "dr", "cr"] if kw in text) >= 3
...
def _parse_row(cells: list[str], account_id: str, account_holder: str, file_path: str) -> Optional[UniversalTransaction]:
    try:
        ...
        logger.debug("Canara row parse error: %s", e)
        return None
...
async def parse_pdf(file_path: str) -> list[UniversalTransaction]:
    ...
    try:
        ...
        logger.debug("Canara PDF parse error: %s", e)
    if not txns:
